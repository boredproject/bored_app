...
interface DropdownMenuLoginProps {
    isOpen: boolean;
    toggleDropdown: () => void;
}

const DropdownMenuLogin: React.FC<DropdownMenuLoginProps> = ({ isOpen, toggleDropdown }) => {
    const [showProfile, setShowProfile] = useState(false);

    const handleLoginClick = () => {
        // Affiche le DropdownMenuProfile
        setShowProfile(true);
        // Ferme le DropdownMenuLogin
        toggleDropdown();
    };
    ...
    return (
        <div className="absolute z-10 w-80 flex flex-col right-0 bg-zinc-900 p-4 shadow-md shadow-indigo-800 border-4 border-indigo-800/100">
            ...
            <input
                type="text"
                className="flex justify-between items-center border-2 border-indigo-800/100 mb-4 text-sm font-light mb-2 text-black"
            />
            <p>Password</p>
            <input
                type="password"
                className="flex justify-between items-center border-2 border-indigo-800/100 mb-4 text-sm font-light text-white-700 mb-2 bg-white text-black"
            />
            <div className="flex justify-between items-center">
                <Link href="/historic">
                <button
                    className="bg-indigo-800 text-white rounded-lg px-4 py-2 font-semibold"
            ...
            {/* Afficher le DropdownMenuProfile s'il est activé */}
            {showProfile && <DropdownMenuProfile isOpen={showProfile} toggleDropdown={toggleDropdown} />}
        </div>
    );
};
