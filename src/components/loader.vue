<template>
  <div :class="{ 'fade-out': showLoader, 'hide': !showLoader }" @click="handleClick" @keydown.window="handleKeyPress" class="fixed w-full h-full bg-black flex flex-col">
    <div
      class="max-w-screen-md mx-auto flex flex-col gap-2 text-center justify-center items-center h-screen uppercase font-bold text-2xl"
    >
      <div class="flex flex-row gap-2 justify-center align-middle text-center">
        <svg
          width="40"
          height="40"
          viewBox="0 0 24 24"
        ...
        <h1 class="text-3xl font-bold pb-5">WARNING-HEALTH AND SAFETY</h1>
      </div>
      <h2>
      ...
      <p class="normal-case">SFX can be found at</p>
      <p class="pb-20 lowercase">www.nintendo.com/healthsafety/</p>
      <p class="animate-bounce-slow">
        Press
        <span
          class="bg-transparent border-2 border-white text-white p-2 rounded-full"
          >any key</span
        >
        to continue
      </p>
...
const handleKeyPress = () => {
  // Lorsqu'une touche est pressée, masquer le loader avec l'animation de fondu
  showLoader.value = false;
};

const handleClick = () => {
  // Lorsqu'un clic est effectué, masquer le loader avec l'animation de fondu
  showLoader.value = false;
};
</script>